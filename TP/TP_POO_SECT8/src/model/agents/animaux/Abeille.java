...
public abstract class Abeille extends Animal implements Hebergeur{
	...
	public void rencontrer(Agent a) {
		...
		else if (a instanceof Varroa && getNiveauSante()!=Etat.Mourant) {
			aggraverEtat();
			parasite = true;
			a.setCoord(getCoord().getX(), getCoord().getY());
		}
	}
	...
	protected void maj() {
		...
		if (this.tours > 10) { // si on a pas manger depuis 10 tours
	...
	protected void seNourrir() {
		if (this.hebergeur != null) {
			rencontrer((Animal)this.hebergeur);
			ameliorerEtat();
			this.tours = 0; //Remise à 0 des tours sans avoir manger
